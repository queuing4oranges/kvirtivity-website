	...
	return (
		<>
			{events && events.map((event, idx) => (
				<Card
					key={idx}
					className='p-0 event-card rounded-3 overflow-hidden'
					onClick={() => (
				...
				>
					<div className='event-image'>
						...
						/>
						<div className='date-square position-absolute top-0 start-0 p-2 bg-white bg-opacity-75  text-center'>
							<span className='d-block fw-bold text-uppercase'>{getMonthName(event.date)}</span>
							<span className='d-block fw-bold'>{getDay(event.date)}</span>
						</div>
					</div>
